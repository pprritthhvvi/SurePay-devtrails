...
```bash
# Option 1: Docker CLI
docker exec -it surepay_postgres psql -U surepay -d surepay_db

# Option 2: pgAdmin UI
# http://localhost:5050
# Email: admin@surepay.local
# Password: surepay_dev_password
```
...
# Restart postgres
docker restart surepay_postgres
```

### Redis Connection Error
```bash
# Verify Redis is running
docker exec surepay_redis redis-cli ping
# Should return: PONG
```
...
```bash
docker build -t surepay-backend:latest backend/
docker run -p 8000:8000 surepay-backend:latest
```
...
- **Slack Channel:** #surepay-devtrails
